...
it('plays bounded expressive gestures then continues its repertoire without moving the seat',()=>{
  ...
  for(const gesture of ['win','loss','wave','groom','lean'] as Gesture[]){
    ...
    expect(extent).toBeGreaterThan(.1);expect(fly.group.position.y).toBe(0);expect(fly.gesture).not.toBe(null);
  }
});
