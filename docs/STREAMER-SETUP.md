...
## OBS broadcast window

Use **http://localhost:4173/?stream=1**, Browser Source **1920×1080**. It contains the desk camera, face-camera inset, brain panel, execution journal and caption/status strip. In OBS click **Interact**, then **Enable Adam voice**. Capture its audio once. Enable speech in only one browser. The primary browser owns trading/learning; an OBS viewer may speak newly saved fills without becoming another trader. Pending audio does not persist across page reloads.
...
Only one turn per voice-enabled browser generates/speaks at a time. Trade announcements take priority after the current sentence; viewer replies remain FIFO. When chat is quiet, the fly describes live book conditions, upcoming prediction checks, scored lessons and its local ledger, with occasional “Hey chat” greetings and synchronized waves. Mute stops playback. A voice backlog in the trading browser pauses new entries but does not block risk exits. Only the primary browser trades; other browsers read its shared ledger and may provide stream audio. Pending speech/chat queues are session memory, not a durable inbox; leave the primary browser instance running. Capture the chosen voice browser's audio in OBS and avoid duplicating it through desktop audio.
...
The monitor changes focus about every twenty seconds. Background scanning retains per-token windows in a rolling 24-contract watchlist, so focus changes do not reset those lessons. Pending lessons expire after gaps, rather than being scored against a much later price. The completed counter advances only after a valid later same-market observation. Open positions hold market focus until an exit; a three-minute maximum hold remains. A bounded $2 momentum-and-tight-spread exploration path supplements learned-edge entries. A descending model spike still gates every fill. Learning does not guarantee profit.
