...
describe('execution safeguards',()=>{
  it('connects LIF motor spikes to a bounded exploration and timed exit',()=>{
    const a=new FlyAgent(),n=new LIFConnectome();let q={...quote(),bid:.499,mid:.4995,spread:.001,bids:[{price:.499,size:100}]};a.observe(q);n.setSensoryFrame(q);n.setDecisionDrive(a.intent(q));
    ...
    vi.advanceTimersByTime(61000);q={...q,timestamp:Date.now()};a.observe(q);expect(a.intent(q).sell).toBe(false);expect(a.memory.lessons).toBe(1);
    vi.advanceTimersByTime(120000);q={...q,timestamp:Date.now()};n.setDecisionDrive(a.intent(q));const outputs=Array.from({length:120},()=>n.step(8.333)).flat();expect(outputs.some(s=>s.neuronId==='DNp02')).toBe(true);
    const exit=a.act('SELL',q,market)!;expect(exit).not.toBeNull();expect(exit.pnl).toBeLessThan(0);expect(a.memory.holdings['123']).toBeUndefined();
  });
  ...
  it('keeps background lessons token-specific across focus switches',()=>{const a=new FlyAgent();a.observe(quote());vi.advanceTimersByTime(61000);a.observe({...quote(),token:'456'});expect(a.memory.lessons).toBe(0);a.observe(quote());expect(a.memory.lessons).toBe(1);});
  it('rejects the observed wide-spread, motionless losing pattern',()=>{const a=new FlyAgent();const q={...quote(),bid:.2,ask:.21,mid:.205,velocity1m:0,spread:.01};a.observe(q);expect(a.intent(q).buy).toBe(false);expect(a.act('BUY',q,market)).toBe(null);});
});
